...
export default function RegisterPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold text-white mb-2">Regisztráció</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            {AGE_GROUPS.map((ag) => (
              <option key={ag} value={ag} className="bg-field-dark text-white">
                {ag} éves
              </option>
            ))}
          </select>
          ...
          {sports.map((s, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <select
                ...
                className="flex-1 px-4 py-2.5 border border-white/20 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none bg-white/10 text-white"
              >
                <option value="" className="bg-field-dark text-white">Válassz sportot</option>
                {SPORTS.map((sp) => (
                  <option key={sp} value={sp} className="bg-field-dark text-white">
                    {sp}
                  </option>
                ))}
              </select>
